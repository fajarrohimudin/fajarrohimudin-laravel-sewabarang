	...
	<main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
		...
		<section id="Details" class="flex flex-col mt-[65px] px-5 w-full gap-5">
			<div id="Heading" class="flex items-center justify-between">
				...
				<div class="flex">
					...
					<p class="font-semibold">{{number_format($product->testimonials->avg('rating'))}}/5 <span class="font-normal text-[#6E6E70]">({{$product->testimonials->count()}})</span></p>
				</div>
			</div>
						...
						<div class="flex gap-3">
							...
							<div class="flex flex-col gap-[6px] w-full">
								<div class="flex items-center">
									<p class="text-sm leading-[21px] font-semibold">{{ $testi->user->name }}</p>
									
									<div class="flex gap-[2px]" style="margin-left: auto;">
										@for ($i = 1; $i <= $testi->rating; $i++)
											<div style="width: 20px; height: 20px;">
												<img src="{{ asset('assets/images/icons/Star.svg') }}" alt="star" style="width: 100%; height: 100%;" />
											</div>
										@endfor
									</div>
								</div>
								<p class="text-xs leading-5 text-[#6E6E70]">{{$testi->ulasan}}</p>
							</div>
						</div>
					</div>
					@empty
						<p class="text-xs leading-5 text-[#6E6E70]">Belum ada testimoni</p>
                	@endforelse
				</div>
			</div>
		</section>
		<div id="Bottom-nav"
			...
			<div class="flex items-center justify-between p-5 relative z-10">
				...
				@else
				<a href="{{ route('filament.admin.auth.login') }}" class="rounded-full p-[12px_24px] bg-[#FCCF2F] font-bold w-fit">Login or register</a>
				@endauth
			</div>
		</div>
	</main>
