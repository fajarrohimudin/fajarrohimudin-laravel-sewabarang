...
@section('content')
		...
		<main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
			<div id="Top-navbar" style="display: flex; align-items: center; justify-content: space-between; padding: 20px;">
				<!-- Logo di kiri -->
				<a href="{{ route('front.index') }}" style="display: flex; align-items: center;">
					<img src="{{ asset('assets/images/logos/logo.svg') }}" alt="logo" style="height: 44px;">
				</a>

				<!-- Notif + User Dropdown di kanan -->
				<div style="display: flex; align-items: center; gap: 1px;">
					<!-- Notifikasi -->
					<a href="#" style="width: 44px; height: 44px; display: flex; align-items: center; justify-content: center;">
						<img src="{{ asset('assets/images/icons/notifications2.svg') }}" alt="Notifications Icon" style="width: 24px; height: 24px;">
					</a>

					<!-- User Dropdown -->
					<div x-data="{ open: false }" style="position: relative;">
						<button @click="open = !open" style="width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; border: none; background: none; cursor: pointer;">
							<img src="{{ asset('assets/images/icons/user.svg') }}" alt="User Icon" style="width: 24px; height: 24px;">
						</button>
						...
						>
							@auth
								<a href="" style="display: block; padding: 10px; color: #333; text-decoration: none; border-bottom: 1px solid #eee;">Profil</a>
								<form method="POST" action="{{ route('logout') }}" style="margin: 0;">
									@csrf
									<button type="submit" style="width: 100%; padding: 10px; text-align: left; background: none; border: none; cursor: pointer; color: #333;">Logout</button>
								</form>
							@else
								<a href="{{ route('filament.admin.auth.login') }}" style="display: block; padding: 10px; color: #333; text-decoration: none; border-bottom: 1px solid #eee;">Login</a>
								<a href="{{ route('filament.admin.auth.register') }}" style="display: block; padding: 10px; color: #333; text-decoration: none;">Register</a>
							@endauth
						</div>
					</div>
				</div>
			...
			<section id="New" class="flex flex-col gap-[10px] mt-[30px]">
				<h2 class="font-semibold text-lg leading-[27px] px-5">Group New</h2>
								...
								<div class="flex flex-col gap-1">
									...
									<div class="flex items-center justify-between">
										...
										<div class="flex items-center gap-[2px]">
											...
											<p class="font-semibold text-sm leading-[21px]">{{number_format($item_latest_product->testimonials->avg('rating'))}}/5</p>
										</div>
									</div>
								</div>
							</div>
			...
			<section id="Recommendation" class="flex flex-col gap-[10px] mt-[30px] px-5">
				...
				<div class="flex flex-col gap-5">
					...
					@forelse($random_products as $item_random_product)
					<a href="{{route('front.details', $item_random_product->slug)}}" class="card">
						<div class="flex items-center gap-3">
							...
							<div class="w-full flex flex-col gap-1">
								...
								<div class="flex items-center justify-between">
									<p class="text-sm leading-[21px] text-[#6E6E70]">Rp {{number_format($item_random_product->price, 0, ',','.')}}/day</p>
									<div class="flex items-center w-fit gap-[2px]">
										<div class="w-4 h-4 flex shrink-0">
											<img src="{{asset('assets/images/icons/Star 1.svg')}}" alt="star" />
										</div>
										<p class="text-sm leading-[21px]"><span class="font-semibold">{{number_format($item_random_product->testimonials->avg('rating'))}}/5</span> <span class="text-[#6E6E70]">({{$item_random_product->testimonials->count()}})</span></p>
									</div>
								</div>
							</div>
						</div>
			...
			<div id="Bottom-nav" class="fixed bottom-0 max-w-[640px] w-full mx-auto border-t border-[#F1F1F1] overflow-hidden z-10">
				...
				<ul class="flex items-center gap-3 justify-evenly p-5 relative z-10">
					...
					<li>
						@php
							$url = auth()->check() ? route('front.transactions') : route('filament.admin.auth.login');
						@endphp
						<a href="{{$url}}">
							<div class="group flex flex-col items-center text-center gap-2 transition-all duration-300 hover:text-black text-[#9D9DAD]">
								...
								<p class="font-semibold text-sm leading-[21px] text-[]">My Orders</p>
							</div>
						</a>
					</li>
					<li>
