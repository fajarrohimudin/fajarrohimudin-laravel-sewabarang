...
@section('content')
		<main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
			<section id="CheckBook" class="w-full flex flex-col gap-10 px-5 items-center pt-20 max-w-[353px] m-auto">
				...
				<div class="flex flex-col gap-5 rounded-[20px] overflow-hidden outline outline-1 outline-[#E9E8ED] p-5 w-full">

					<!-- Label Judul -->
					<label for="rating" class="font-semibold">
						{{ $testimonial ? 'Ulasan Anda' : 'Bagaimana order yang diberikan?' }}
					</label>
					...
					<div class="flex gap-1" id="starContainer">
						@for ($i = 1; $i <= 5; $i++)
							<img src="{{ asset('assets/images/icons/Star.svg') }}"
								...
								alt="star"
								class="w-7 h-7 
									{{ $testimonial && $i <= $testimonial->rating ? '' : 'filter grayscale' }} 
									{{ $testimonial ? '' : 'cursor-pointer star-img' }}" />
						@endfor
					</div>
					...
					<div class="flex flex-col gap-2">
						...
						<div class="group w-full rounded-2xl border border-[#EDEEF0] p-[18px_14px] flex items-start gap-3 relative">
							...
							<textarea
								...
								{{ $testimonial ? 'readonly' : 'required' }}
							>{{ $testimonial ? $testimonial->ulasan : '' }}</textarea>
						</div>
					</div>
