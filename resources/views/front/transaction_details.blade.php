    ...
    <main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
            ...
            <div id="Payment-details" class="flex flex-col gap-3">
                ...
                <div class="flex flex-col gap-4">
                    <div class="flex items-center justify-between">
                        <p>Price</p>
                        <p class="font-semibold">Rp {{number_format($productPrice, 0, ',','.')}}</p>
                    </div>
                    <div class="flex items-center justify-between">
                        <p>Durasi sewa</p>
        ...
        <div id="Bottom-nav" class="fixed bottom-0 max-w-[640px] w-full mx-auto border-t border-[#F1F1F1] overflow-hidden z-10">
            ...
            <div class="flex items-center justify-between p-5 relative z-10">
                @if($details->is_paid)
                    @if($details->testimonials && $details->id == $details->testimonials->transaction_id)
                        <a href="{{ route('front.testimonials.show', $details->testimonials->id) }}" class="rounded-full p-[12px_24px] bg-[#FCCF2F] font-bold w-full text-center">
                            Lihat Ulasan
                        </a>
                    @else
                        <a href="{{ route('front.testimonials') }}" class="rounded-full p-[12px_24px] bg-[#FCCF2F] font-bold w-full text-center">
                            Beri Ulasan
                        </a>
                    @endif
                @else
                    <a href="{{ $details->transaction_url }}" target="_blank" class="rounded-full p-[12px_24px] bg-[#FCCF2F] font-bold w-full text-center">
                        Continue Payment
                    </a>
                @endif
            </div>
