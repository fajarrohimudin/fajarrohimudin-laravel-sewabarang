		...
		<main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
			<section id="CheckBook" class="w-full flex flex-col gap-10 px-5 items-center pt-20 max-w-[353px] m-auto">
				...
				<div class="flex flex-col gap-2 items-center">
					<h1 class="text-2xl leading-[36px] font-bold">Feedback Order</h1>
					<p class="leading-[30px] text-[#6E6E70] text-center">Berikan rating order secara jujur dan details</p>
				</div>
				...
				<form method="POST" action="{{route('front.testimonials.send')}}" class="flex flex-col gap-5 rounded-[20px] overflow-hidden outline outline-1 outline-[#E9E8ED] p-5 w-full">
					@csrf
					...
					<!-- Bintang Rating -->
					<label for="phone" class="font-semibold">Bagaimana order yang diberikan?</label>
					<div class="flex gap-1" id="starContainer">
						@for ($i = 1; $i <= 5; $i++)
							<img src="{{ asset('assets/images/icons/Star.svg') }}" 
								data-star="{{ $i }}" 
								alt="star" 
								class="w-7 h-7 cursor-pointer star-img filter grayscale" />
						@endfor
					</div>

					<!-- Textarea Ulasan -->
					<div class="flex flex-col gap-2">
						<label for="ulasan" class="font-semibold">Ulasan</label>
						<div class="group w-full rounded-2xl border border-[#EDEEF0] p-[18px_14px] flex items-start gap-3 relative transition-all duration-300 focus-within:ring-2 focus-within:ring-[#FCCF2F]">
							...
							<textarea
								...
								id="ulasan"
								rows="4"
								class="appearance-none outline-none w-full resize-none placeholder:font-normal placeholder:text-black font-semibold text-sm leading-[24px]"
					...
					<!-- Submit Button -->
					<button type="submit" class="rounded-full p-[12px_24px] bg-[#FCCF2F] font-bold w-full text-center">
						Kirim Ulasan
					</button>
				</form>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Atur default rating ke 5
        const defaultRating = parseInt(ratingInput.value);

        // Tampilkan bintang sesuai default
        stars.forEach((s, i) => {
            if (i < defaultRating) {
                s.classList.add('yellow');
                s.classList.remove('grayscale');
            } else {
                s.classList.add('grayscale');
                s.classList.remove('yellow');
            }
        });

        // Saat user klik bintang
